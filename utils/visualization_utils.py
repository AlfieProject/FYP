...
def return_coordinates(
  ...
  # Calculate coordinates of each piece.
  coords = []
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box
    height, width, channel = image.shape
    y = int(((ymin * height) + (ymax * height)) / 2)
    y_adjusted = int(y + (0.2 * ((ymax * height) - (ymin * height))))
    x = int(((xmin * width) + (xmax * width)) / 2)
    coords.append([x, y_adjusted, (box_to_score_map[box] * 100), (box_to_display_str_map[box][0])])
